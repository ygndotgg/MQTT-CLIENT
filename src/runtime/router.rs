...
impl Router {
    pub fn add_subscription(&mut self, client_id: usize, filter: String) {
        ...
        self.entries.push(SubscriptionEntry { filter, clients: vec![client_id] });
    }
...
#[cfg(test)]
    ...
    pub fn test_subscription() {
        ...
        let subscribers: Vec<&SubscriptionEntry> = router
            .entries
            .iter()
            .map(|x| {
                assert_eq!(x.clients, vec![1, 2]);
                x
            })
            .collect();
    }
    #[test]
    pub fn matching_clients() {
        let mut router = Router::default();

        router.add_subscription(1, "sensor/+".to_string());

        router.add_subscription(2, "sensor/+".to_string());
        assert_eq!(router.matching_clients("sensor/+"), vec![1, 2])
    }
...
#[test]
      fn router_add_subscription_deduplicates_client() {
          ...
          assert_eq!(router.matching_clients("sensor/temp"), vec!
  [1]);
      }
      ...
      fn router_remove_subscription_drops_empty_entry() {
          ...
          assert!(router.matching_clients("sensor/
  temp").is_empty());
      }
  ...
  2]);
      ...
      fn router_matching_clients_single_level_wildcard() {
          ...
          assert_eq!(router.matching_clients("sensor/temp"), vec!
  [1]);
          assert!(router.matching_clients("sensor/temp/
  outdoor").is_empty());
      }
      ...
      fn router_matching_clients_multi_level_wildcard() {
          ...
          assert_eq!(router.matching_clients("alerts/high/cpu"),
  vec![1]);
    }
}
}
